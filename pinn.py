...
def train_and_evaluate_pinn():
    ...
    # ==============================================================================
    # PARTS 8 & 9: NATIONAL MAPPING GRID ESTIMATION (WITH STRUCTURAL CONTEXTS)
    # ==============================================================================
    print("\n=== STEP 8 & 9: Building National Predictive Temperature Mesh ===")
    lats_arr = np.arange(np.floor(lat_min), np.ceil(lat_max), 0.1)  # High-resolution spatial grid
    lons_arr = np.arange(np.floor(lon_min), np.ceil(lon_max), 0.1)
    lon_m, lat_m = np.meshgrid(lons_arr, lats_arr)
    
    grid_df = pd.DataFrame({'lat': lat_m.flatten(), 'lon': lon_m.flatten()})
    
    # Clip grid layout to country boundary limits
    gdf_map_init = gpd.GeoDataFrame(grid_df, geometry=gpd.points_from_xy(grid_df['lon'], grid_df['lat']), crs="EPSG:4326")
    clipped_gdf = gpd.sjoin(gdf_map_init, india_border, how="inner", predicate="within").drop_duplicates(subset=['lat', 'lon'])
    grid_df = pd.DataFrame(clipped_gdf.drop(columns='geometry')).reset_index(drop=True)
    ...
    for idx, row in grid_df.iterrows():
        # Match nearest training borehole coordinate to copy realistic continuous features
    ...
    map_spatial_raw[:, 2] = 0.1  # Evaluate slightly below surface sheet layer to extract gradients
    ...
    # Smooth local noise within reasonable literature constraints
    grid_df['predicted_q'] = grid_df['predicted_q'].clip(df[target_col].min() - 5, df[target_col].max() + 5)

    # ==============================================================================
    # PART 10: PUBLICATION LAYOUT EXPORT (MATCHING TARGET REFERENCE FIGURE)
    # ==============================================================================
    ...
    # Exact color intervals matching your target reference document (14 to 126 mW/m²)
    ...
    # Render regional variations using contour fills
    contour = ax.contourf(pivot_q.columns, pivot_q.index, pivot_q.values, levels=bounds, cmap=custom_cmap, norm=norm, extend='both', alpha=0.9)
    ax.contour(pivot_q.columns, pivot_q.index, pivot_q.values, levels=bounds, colors='black', linewidths=0.15, alpha=0.4)
    
    # Plot real training borehole locations to verify data anchor alignments
    ax.scatter(df['lon'], df['lat'], c='magenta', edgecolor='black', s=16, linewidths=0.5, zorder=5, label='Borehole Data Locations')
    
    # Clear national shoreline boundaries
    india_border.plot(ax=ax, facecolor='none', edgecolor='#111111', linewidth=1.3, zorder=4)
    
    ax.set_xlim(lon_min - 0.5, lon_max + 0.5)
    ax.set_ylim(lat_min - 0.5, lat_max + 0.5)
    ax.set_title("Deep PINN Reconstruction: Geothermal Heat Flow Map of India", pad=20, weight='bold', fontsize=14)
    ...
    cbar.set_label("Derived Heat Flow ($q = -k\\nabla T$, $\\text{mW/m}^2$)", rotation=270, labelpad=25, weight='bold', fontsize=11)
    cbar.set_ticks(bounds)
    
    os.makedirs("national_outputs", exist_ok=True)
    plt.tight_layout()
    plt.savefig("national_outputs/india_pinn_heat_flow_field.png", bbox_inches='tight')
    plt.close()
    print("Map successfully exported: './national_outputs/india_pinn_heat_flow_field.png'")
